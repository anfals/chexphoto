    ...
    def __init__(self, data_df, data_dir, transform):
        ...
        for index, row in data_df.iterrows():
            ...
            if os.path.exists(file_path):
                ...
                label = torch.zeros((5,))
                for index, observation in enumerate(CheXBudget.observations):
...
def fetch_dataloader(types, data_dir, params):
    ...
    for index, split in enumerate(['train', 'val', 'test']):
        if split in types:
            df = dfs[index]
            dl = DataLoader(CheXBudget(df, data_dir, preprocess), batch_size=params.batch_size, shuffle=True,
